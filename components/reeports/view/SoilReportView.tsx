...
const SoilReportView: React.FC<SoilReportViewProps> = ({ report }) => {
  const { institute } = useInstitute();
  
  // Fixed: exactly 2 samples per page (6 rows total - 3 rows per sample)
  const samplesPerPage = 2;
  const totalSamples = report.samples?.length || 0;
  const totalPages = Math.ceil(totalSamples / samplesPerPage);

  console.log('SoilReportView pagination:', {
    totalSamples,
    samplesPerPage,
    totalPages,
    rowsPerPage: samplesPerPage * 3
  });
        ...
        >
          <div className="absolute top-2 right-4">
            <p className="text-xs font-medium text-black">
              পৃষ্ঠা: {toBanglaNumber(pageIndex + 1)}/{toBanglaNumber(totalPages)}
            </p>
          </div>
              ...
              <Table className="border-collapse border-black">
                <TableHeader>
                  <TableRow className='border-l border-r border-t border-black'>
                    ...
                    >
                      বিশ্লেষিত ফলাফল ও উর্বরতা শ্রেনী
                    </TableHead>
                  </TableRow>
                ...
                <TableBody className='border-b border-r border-l border-black'>
                  {getSamplesForPage(pageIndex).map((sample) => (
                    <React.Fragment key={sample.id}>
                      {/* Main Row with Values */}
                      <TableRow className='border-black'>
                        <TableCell rowSpan={3} className='border-r border-black text-black text-xs p-2 vertical-align-top'>
                          {sample.sampleIdNumber}
                        </TableCell>
                        <TableCell rowSpan={3} className='border-r border-black text-black text-xs p-2 vertical-align-top'>
                          {report.client?.name}
                        </TableCell>
                        <TableCell className='border-r border-black text-black text-xs p-2'></TableCell>
                      ...
                      <TableRow className='border-black'>
                        <TableCell className='border-r border-black text-black text-xs p-2'>আপল্যান্ড</TableCell>
                        {sample.orderItem?.orderTestParameters?.map((param) => {
                          const testResult = sample.testResults?.find(tr => tr.testParamater.id === param.testParameter.id);
                          return (
                            <TableCell key={param.id} className='border-r border-black text-center text-black text-xs p-2'>
                              {testResult?.uplandInterpretation || '-'}
                            </TableCell>
                          );
                        })}
                      </TableRow>
                      ...
                      <TableRow className='border-black'>
                        <TableCell className='border-r border-black text-black text-xs p-2'>ওয়েটল্যান্ড</TableCell>
                        {sample.orderItem?.orderTestParameters?.map((param) => {
                          const testResult = sample.testResults?.find(tr => tr.testParamater.id === param.testParameter.id);
                          return (
                            <TableCell key={param.id} className='border-r border-black text-center text-black text-xs p-2'>
                              {testResult?.wetlandInterpretation || '-'}
                            </TableCell>
                          );
                        })}
                      </TableRow>
